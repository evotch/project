<h1 class="page-header">Color Proportions</h1>
<p>Accents (backgrounds), Supporting (backgrounds), Subordinate (reinforece the message), Brown, Accent Blue, Dominant (convey the message), Navy, Marine, Green, 
</p>

<div class="progress">
  <div class="progress-bar progress-bar-pink" style="width: 4%">
    <span class="sr-only">10% Complete (danger)</span>
  </div>
    <div class="progress-bar progress-bar-white" style="width: .5%">
    <span class="sr-only">10% Complete (danger)</span>
  </div>
    <div class="progress-bar progress-bar-plum" style="width: 4%">
    <span class="sr-only">10% Complete (danger)</span>
  </div>
      <div class="progress-bar progress-bar-white" style="width: .5%">
    <span class="sr-only">10% Complete (danger)</span>
  </div>
  <div class="progress-bar progress-bar-gold" style="width: 4%">
    <span class="sr-only">10% Complete (danger)</span>
  </div>
      <div class="progress-bar progress-bar-white" style="width: .5%">
    <span class="sr-only">10% Complete (danger)</span>
  </div>
  <div class="progress-bar progress-bar-orange" style="width: 4%">
    <span class="sr-only">10% Complete (danger)</span>
  </div>
      <div class="progress-bar progress-bar-white" style="width: 3%">
    <span class="sr-only">10% Complete (danger)</span>
  </div>
  <div class="progress-bar progress-bar-aqua" style="width: 6%">
    <span class="sr-only">6% Complete (danger)</span>
  </div>
        <div class="progress-bar progress-bar-white" style="width: .5%">
    <span class="sr-only">10% Complete (danger)</span>
  </div>
      <div class="progress-bar progress-bar-cream" style="width: 14%">
    <span class="sr-only">10% Complete (danger)</span>
  </div>
    <div class="progress-bar progress-bar-white" style="width: .5%">
    <span class="sr-only">10% Complete (danger)</span>
  </div>
      <div class="progress-bar progress-bar-tan" style="width: 8%">
    <span class="sr-only">10% Complete (danger)</span>
  </div>
    <div class="progress-bar progress-bar-white" style="width: .5%">
    <span class="sr-only">10% Complete (danger)</span>
  </div>
      <div class="progress-bar progress-bar-tan" style="width: 3%">
    <span class="sr-only">10% Complete (danger)</span>
  </div>
      <div class="progress-bar progress-bar-white" style="width: 3%">
    <span class="sr-only">10% Complete (danger)</span>
  </div>
  <div class="progress-bar progress-bar-blue" style="width: 4%">
    <span class="sr-only">10% Complete (danger)</span>
  </div>
      <div class="progress-bar progress-bar-white" style="width: .5%">
    <span class="sr-only">10% Complete (danger)</span>
  </div>
    <div class="progress-bar progress-bar-brown" style="width: 4%">
    <span class="sr-only">35% Complete (success)</span>
  </div>
        <div class="progress-bar progress-bar-white" style="width: .5%">
    <span class="sr-only">10% Complete (danger)</span>
  </div>
    <div class="progress-bar progress-bar-green" style="width: 9%">
    <span class="sr-only">35% Complete (success)</span>
  </div>
        <div class="progress-bar progress-bar-white" style="width: .5%">
    <span class="sr-only">10% Complete (danger)</span>
  </div>
  <div class="progress-bar progress-bar-teal" style="width: 8%">
    <span class="sr-only">20% Complete (warning)</span>
  </div>
      <div class="progress-bar progress-bar-white" style="width: .5%">
    <span class="sr-only">10% Complete (danger)</span>
  </div>
      <div class="progress-bar progress-bar-navy" style="width: 17%">
    <span class="sr-only">10% Complete (danger)</span>
  </div>



</div>

